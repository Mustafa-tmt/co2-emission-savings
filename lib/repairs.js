 ...
 * @param {'conservative'|'central'|'optimistic'} [replacementScenario]
 */
function analyzeRepair(
  ...
  componentRows,
  partCatalogMap,
  replacementScenario = DEFAULT_SCENARIO
) {
  const warnings = [];
  const hardErrors = [];
  const lines = [];
  ...
  const scenario = normalizeReplacementScenario(replacementScenario);
  const mCo2 = Number(device.manufacturing_co2);
  if (!Number.isFinite(mCo2)) {
    warnings.push(
      'Invalid or missing manufacturing_co2 for device; manufacturing baseline treated as 0.'
    );
  }

  const co2 = calculateAvoidedCo2(Number.isFinite(mCo2) ? mCo2 : 0, partsCo2, scenario);
  ...
  return {
    ...
    manufacturingBaseline: co2.manufacturingBaseline,
    repairBurden: co2.repairBurden,
    avoidedKg: co2.avoidedKg,
    replacementScenario: co2.scenario,
    replacementProbabilityP: co2.P,
    warnings,
    hardErrors,
  };
}
 ...
 * @param {{ replacementScenario?: string }} [evalOptions]
 */
function evaluateRepairJob(rawRow, ctx, evalOptions = {}) {
  ...
  const replacementScenario = normalizeReplacementScenario(
    evalOptions && evalOptions.replacementScenario
  );
